...
public class ReturnServiceImpl implements ReturnService {
    ...
    @Override
    public ReturnResponseDto updateReturnStatus(Long id, ReturnStatus status) {
...
AReturn.setStatus(status);
        if(status == ReturnStatus.APPROVED || status == ReturnStatus.REJECTED || status == ReturnStatus.COMPLETED){
            AReturn.setProcessedAt(java.time.LocalDateTime.now());
        }
        Return updated = returnRepository.save(AReturn);
        return ReturnMapper.toResponse(updated);
    }

    private boolean isValidStatusTransition(ReturnStatus currentStatus, ReturnStatus status) {
